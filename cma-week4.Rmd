...
ws %>% group_by(TierName) %>% summarise(n =)
...
ws %>% 
  ...
  reduce(left_join, by = "dt_rounded") %>% 
  mutate(distance = -> ws_joined #this method splits all - not what we want
...
ggplot() +
  ...
  geom_point(data = ws_join %>%  filter(meet == TRUE),
             aes(x=E_rosa, y=N_rosa, fill = TierName_rosa), pch = 21, color = "black") +
  theme_void()
